...
function registerValidate() {
    ...
    if (inputZip.value == "") {
        ...
        document.getElementById("errorZip").textContent = "Camp obligatori";
        acumErrores++;
    } else if (!validar_codipostal(inputZip.value)) {
        inputZip.classList.add("is-invalid");
        document.getElementById("errorZip").textContent = "Format incorrecte, ha de ser 5 dígits";
        acumErrores++;
    }
...
function validar_codipostal(codi) {
    var regex = /^[0-5][1-9]{3}[0-9]$/;
    return regex.test(codi) ? true : false;
}